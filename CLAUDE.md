...
- Use optimistic concurrency for mutable hot-path entities.
- Use transactions for multi-step state transitions.
- Flow cancellation tokens through data access.
- Startup auto-applies migrations and idempotent seed data.
- Create/remove migrations only with `dotnet ef` CLI.
